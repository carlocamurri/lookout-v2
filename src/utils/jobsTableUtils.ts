...
const fetchRows = async (rowRequest: FetchRowRequest, getJobsService: GetJobsService, groupJobsService: GroupJobsService, selectedColumns: ColumnSpec[], currentGroupedColumns: string[]): Promise<FetchRowResult> => {
    ...
    if (groupingLevel === expandedLevel) {
        ...
        const filters: JobFilter[] = parentRowId?.split(">").map(s => s.split(":"))
            .map(([field, value]) => ({
                field,
                value,
                match: "exact"
...
export const fetchAllNeededRows = async (rowRequests: FetchAllNeededRowsRequest, existingData: JobTableRow[], getJobsService: GetJobsService, groupJobsService: GroupJobsService, selectedColumns: ColumnSpec[], currentGroupedColumns: string[]): Promise<FetchAllNeededRowsResult> => {
    ...
    const mergedData: JobTableRow[] = responses.reduce<JobTableRow[]>((partiallyMergedData, response) => {
        ...
        const rowIdsPathForParent = parentToFind?.split(">").reduce<string[]>(
            (paths, newLevel) => {
